...
export class TableLogger {
	...
	constructor(options: Partial<Options> = {}) {
		const { columns = 3, pending = 2, align = '', loglevel = 'error' } = options
		this.options = {
			columns,
			pending,
			align,
	...
	error(string = '') {
		if (shouldLog(this.options.loglevel, 'error')) this.rows.push(string)
	}

	warn(string = '') {
		if (shouldLog(this.options.loglevel, 'warn')) this.rows.push(string)
	}

	debug(string = '') {
		if (shouldLog(this.options.loglevel, 'debug')) this.rows.push(string)
	}
	...
	output() {
		...
		this.rows.forEach((line) => {
			...
			for (let i = 0; i < columns; i++) columnsWidth[i] = Math.max(columnsWidth[i], visualLength(line[i] || ''))
		})
...
export function wrapJoin(strs: string[], delimiter: string, width: number): string[] {
	...
	for (let i = 0; i < strs.length; i++) {
		...
		line += str
		if (i < strs.length - 1) line += delimiter
	}
	lines.push(line)
	return lines
}
