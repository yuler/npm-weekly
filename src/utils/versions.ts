...
export function getVersionRangePrefix(v: string) {
	...
	for (const leading of leadings) {
		if (ver.startsWith(leading)) return leading
	}
...
export function getPrefixedVersion(current: string, target: string) {
	...
	return applyVersionRangePrefix(target, prefix)
}
...
export function getMaxSatisfying(
	...
	if (mode === 'latest') {
		...
		if (!semver.satisfies(maxVersion, range)) maxVersion = null

		versions.forEach((ver) => {
			if (semver.satisfies(ver, range)) {
				if (!maxVersion || semver.lte(ver, maxVersion)) version = ver
			}
		})
	}
