			...
			if (options.detail) {
				...
				for (const [version, pkgs] of Object.entries(versionMap)) {
					...
					lines.forEach((line, i) => {
						if (i === 0)
							logger.log(
								`${visualPadStart(c.gray(colorizeVersionDiff(latest || version, version, false)), pad, ' ')}  ${line}`
							)
						else logger.log(`${' '.padStart(pad, ' ')}  ${line}`)
					})
				}
			} else {
				logger.row(
