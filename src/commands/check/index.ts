...
export async function check(options: CheckOptions) {
	...
	if (options.interactive) resolvePkgs = await promptInteractive(resolvePkgs, options)

	const hasChanges = resolvePkgs.length && resolvePkgs.some((i) => i.resolved.some((j) => j.update))
	if (!hasChanges) return console.log(c.green('dependencies are already up-to-date'))
	...
	if (!options.all) {
		const counter = resolvePkgs.reduce((counter, pkg) => {
			for (let i = 0; i < pkg.resolved.length; i++) {
				if (pkg.resolved[i].update) return ++counter
			}
			return counter
		}, 0)
		...
		if (last === 1) console.log(c.green('dependencies are already up-to-date in one package\n'))
		else if (last > 0) console.log(c.green(`dependencies are already up-to-date in ${last} packages\n`))
	}
	...
	if (options.write) {
		for (const pkg of resolvePkgs) await writePackage(pkg, options)
	}
	...
	if (!options.write) {
		...
		if (options.mode === 'default') console.log(`Run ${c.cyan('taze major')} to check major updates`)

		if (hasChanges) console.log(`Add ${c.green('-w')} to write package.json`)

		console.log()
	} else if (hasChanges) {
		if (!options.install && !options.update && !options.interactive) {
			console.log(c.yellow(`ℹ changes wrote to package.json, run ${c.cyan('npm i')} to install updates.`))
		}
