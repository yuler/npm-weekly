...
export async function promptInteractive(pkgs: PackageMeta[], options: CheckOptions) {
	...
	if (!pkgs.some((i) => i.resolved.some((i) => i.update))) return []
	...
	function createListRenderer(): InteractiveRenderer {
		...
		return {
			render() {
				...
				console.clear()
				console.log(
					`${FIG_BLOCK} ${c.gray(`${Y('↑↓')} to select, ${Y('space')} to toggle, ${Y('→')} to change version`)}`
				)
	...
	function createVersionSelectRender(dep: ResolvedDepChange): InteractiveRenderer {
		const versions = Object.entries({
			...
			.map(([name, version]) => {
				...
				if (!targetVersion || targetVersion === dep.currentVersion) return undefined
				return {
					name,
					version,
		...
		return {
			render() {
				...
				console.log(
					`${FIG_BLOCK} ${c.gray(
						`Select a version for ${c.green(c.bold(dep.name))}${c.gray(` (current ${dep.currentVersion})`)}`
					)}`
				)
				console.log()
				console.log(
	...
	function registerInput() {
		...
		process.stdin.on('keypress', (str: string, key: TerminalKey) => {
			...
			if (result && typeof result !== 'boolean') renderer = result
