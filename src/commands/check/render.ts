...
export function renderChange(change: ResolvedDepChange, interactive?: InteractiveContext) {
	...
	const pre = interactive
		? [isSelected ? FIG_POINTER : FIG_NO_POINTER, change.interactiveChecked ? FIG_CHECK : FIG_UNCHECK].join('')
		: ' '
	...
	return [
		...
		change.latestVersionAvailable &&
		semver.minVersion(change.targetVersion)!.toString() !== change.latestVersionAvailable
			? c.dim(c.magenta(`(${change.latestVersionAvailable} available)`))
			: '',
	]
}

export function renderChanges(pkg: PackageMeta, options: CheckOptions, interactive?: InteractiveContext) {
	const { resolved, relative: filepath } = pkg
	const lines: string[] = []
	const errLines: string[] = []

	const changes = options.all ? resolved : resolved.filter((i) => i.update)
	...
	if (changes.length) {
		...
		changes
			...
			.forEach(({ diff }) => {
				if (!diff) return
				if (!diffCounts[diff]) diffCounts[diff] = 0
				diffCounts[diff] += 1
			})
	...
	if (errors.length) {
		lines.push()
		for (const dep of errors) errLines.push(...renderResolveError(dep))
		lines.push()
	}
...
function renderResolveError(dep: ResolvedDepChange) {
	...
	if (dep.resolveError == null) return lines
